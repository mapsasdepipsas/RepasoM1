...
let signozodiaco = zodiac(day, month)
...
function continent(country) {
    let europa = ["España", "Francia", "Alemania"];
    if (europa.includes(country)){
        return "Europa";
    }
    let asia = ["India", "Japón", "China"];
    if (asia.includes(country)){
        return "Asia";
    }
    let america = ["Canada", "Mexico", "Colombia"];
    if (america.includes(country)) {
        return "America";
    }
    let africa = ["Senegal", "Nigeria", "Madagascar"];
    if (africa.includes(country)) {
        return "Africa";
    }
    let oceania = ["Australia", "Nueva Zelanda", "Polinesia Francesa"];
    if (oceania.includes(country)) {
        return "Oceania";
    }
}

let country = "Polinesia Francesa";
let continente = continent(country);
